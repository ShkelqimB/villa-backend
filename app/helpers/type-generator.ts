...
const {STRING, TEXT, INTEGER, DATE, NOW, BOOLEAN, DECIMAL, ENUM, VIRTUAL, FLOAT, JSON} = DataTypes;
...
export const Type = (
    {
        ...
        str: (len: number, allowNull: boolean = true) => ({type: STRING(len), allowNull}),
        intRange: (...range: string[]) => ({type: INTEGER, validate: {isIn: [range]}}),
        uniqInt: () => ({type: INTEGER, allowNull: false, unique: true}),
        isoCode: (len: number, cb: Function) => ({type: STRING(len), set: cb}),
        enum: (...values: string[]) => ({type: ENUM, values}),
        bool: (def?: boolean) => ({type: BOOLEAN, allowNull: true, defaultValue: def || false}),
        date: () => ({type: DATE, allowNull: true}),
        autoDate: () => ({type: DATE, allowNull: false, defaultValue: NOW}),
        virtual: (get = () => {}) => ({type: VIRTUAL(Type.int), get}),
        ...
        toUpperCase: (column: string) => function (val: string) {
            // @ts-ignore
